...
- **A notification is written now; only the outbound channels wait** (#101). F12.4's quiet hours are *"nobody wants a 6am push"* — a rule about being **woken**, and a row appearing in a panel wakes nobody. So `notifications` is written immediately and `deliver_after` holds the email and the push. That is what makes *"delayed, not dropped"* true without a second table, and it means somebody opening the app at 7am has already been told. Evaluated in the **team's** timezone, wall-clock: a stored UTC instant is an hour wrong twice a year, which is exactly the season the rule is judged on. A wrapping window (21:00 → 07:00) must advance the day, or a 22:00 notification is held until an instant fifteen hours past and released immediately.
  - **The channel fan-out is one place.** Five features raise notifications; if each decided its own channels then a preference set on S78 would be honoured by four of them. `App\Support\Notifications\Notify` is the only writer, and a caller says what happened and who should know, nothing else.
