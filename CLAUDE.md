...
| `php artisan mail:suppression <email>` | Show whether an address is on the account-wide suppression list, and why. `--lift` removes it (every team is affected — it is deliberately not something a team can do to itself); `--suppress` adds one by hand. Audited |
