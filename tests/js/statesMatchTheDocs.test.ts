...
function iaStates(heading: string): Record<string, string> {
    ...
    const vocabulary = (doc('Information Architecture').split('## 8. State vocabulary')[1] ?? '').split(
        '\n## ',
    )[0];
    const section = (vocabulary.split(`### ${heading}`)[1] ?? '').split('### ')[0];
...
function designSystemTones(entity: string): Record<string, Tone> {
    const table = doc('Design System').split('### 2.4 State mapping')[1]?.split('###')[0] ?? '';
    ...
    for (const line of table.split('\n')) {
        ...
        if (current.toLowerCase() === entity.toLowerCase() && /^(neutral|info|success|warning|danger)$/.test(tone)) {
...
const DOMAINS: Array<{ domain: StateDomain; ia: string; designSystem: string }> = [
    ...
    { domain: 'automation', ia: 'Automation / message', designSystem: 'Message' },
    { domain: 'extractedField', ia: 'Extracted field', designSystem: 'Extracted field' },
];

describe('the state table matches the documents', () => {
    it.each(DOMAINS)('has IA §8’s codes and labels for $domain', ({ domain, ia }) => {
        const documented = iaStates(ia);
        const implemented = Object.fromEntries(
            Object.entries(STATES[domain]).map(([code, descriptor]) => [code, descriptor.label]),
        );

        expect(implemented).toEqual(documented);
    });

    it.each(DOMAINS)('has Design System §2.4’s tone for $domain', ({ domain, designSystem }) => {
        const documented = designSystemTones(designSystem);

        expect(Object.keys(documented).length, `no §2.4 rows for ${designSystem}`).toBeGreaterThan(0);

        for (const descriptor of Object.values(STATES[domain])) {
            expect(documented[descriptor.label], `${designSystem} · ${descriptor.label}`).toBe(
                descriptor.tone,
            );
        }
    });
    ...
    it('carries the document state from Design System §2.4', () => {
        ...
        expect(Object.entries(STATES.document).map(([, d]) => [d.label, d.tone])).toEqual(
