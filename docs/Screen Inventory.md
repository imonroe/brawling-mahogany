...
| ID | Screen | Route | User | Key states | PRD | Slice | Effort |
|---|---|---|---|---|---|---|---|
| S65 | Extract from document | modal | TC | Upload, queued, processing, failed, cost warning. **Built** (#115). Opens from a document row on S52; the row offers *Review* rather than *Extract* once there is something to look at, and neither while one is running — a second press would spend twice and produce every proposal twice. The dialog names what leaves the account rather than implying redaction is a guarantee (PRD §14.3) | F10.1 | 5 | M |
| S66 | Review extracted dates | `/deals/{deal}/extractions/{extraction}` | TC | Source passage beside proposed dates, confidence levels, edit, reject, conflict with existing, cascade preview. **Built** (#116). **One page renders S66 and S67**, discriminated by `extractions.kind` — the route below is the same route. Known gap: the left pane renders the **verbatim passage the model was required to copy**, not a rendered page, because this project has no PDF viewer and the bytes are on a private disk. The screen says so rather than implying otherwise. §7.4's *"jump the left pane to the highlighted region"* wants a real pdf.js pane | F10.2 | 5 | **L** |
| S67 | Review extracted tasks | `/deals/{deal}/extractions/{extraction}` | Team | Proposed tasks, bulk accept, edit, assign, reject trivia. **Built** (#117). Bulk accept is refused **on the server** for a contract (`AcceptExtractedFieldsRequest::authorize()`), keyed on the kind — a rule stated only in a component is a rule the next caller lacks. Even here it is not *accept all*: the ids are named, so accepting a batch means the client listed what it was accepting | F10.3 | 5 | M |
| S68 | Extraction history | `/settings/extractions` | Admin | Per-deal audit, model version, cost, what the human changed. **Built** (#118). It deliberately does **not** report *critical dates missed*: the application can never know what a contract contained but the model did not report, so a zero there would be a number it cannot see. That metric is `php artisan extraction:score` against hand-checked ground truth, and the screen says where to get it | F10.4 | 5 | S |
