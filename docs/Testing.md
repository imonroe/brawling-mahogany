...
| `tests/Feature/Automations/SendingAutomationsTest.php` (claim cases) | The three that settle what a worker may say about a row it does not own: it **never** narrates an outcome for a claimed row however old the claim looks; the reaper records one from a distance; and the reaper leaves a claim it is not yet sure about. Plus the ones holding the ordering — a halt must not overwrite a stopped message's typed reason or write a rail error onto a delivered one, and the ownership gate must cover **every** action type, since a stopped `create_task` was carried out anyway and a redelivered one made the task twice. The reaper's two records are pinned separately: an audit entry, which an ordinary refusal does not get, and a timeline sentence that does not say *"did not go out"* about a message that may have arrived | PRD §4.5, §9, issue #92 |
