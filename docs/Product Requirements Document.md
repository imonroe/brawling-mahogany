...
| SES production access | **Unconfirmed.** A verified domain identity and a production account are different grants — in the sandbox an account may only send to addresses it has also verified, so a message to a real client is rejected rather than delivered, and the two are indistinguishable from inside the application. Check the account's sending status before the first real send |
