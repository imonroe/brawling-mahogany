...
**Where this actually stands (2026-08-26, #12).** SES is configured and `monroedigitalconsulting.com` is verified; every message leaves as `goldieflow@monroedigitalconsulting.com` over SMTP. Of the requirements above, **one is met and the rest are partly met, unconfirmed, or not built** — and the gaps are worth naming rather than leaving to be discovered, because most of them look identical from inside this application right up to the moment a client should have received something:
...
| SPF, DKIM, DMARC | **DKIM: likely.** SES verifies a domain through Easy DKIM CNAMEs, so `d=monroedigitalconsulting.com` signing is probably real. **SPF alignment and DMARC: unconfirmed, and neither follows from the other.** SPF is evaluated against the envelope MAIL FROM, and without a custom MAIL FROM domain in SES the Return-Path is `*.amazonses.com` — so the domain's own SPF record is never consulted and SPF is unaligned for DMARC, leaving DMARC to ride on DKIM alignment alone. DMARC itself is a `_dmarc` TXT record, entirely outside SES setup. #12's evidence says a domain is approved for sending and nothing about any of the three; check them in DNS before the first real send |
