...
| 2026-08-26 | **A claimed-but-unfinished send is called abandoned only once the claim is older than the queue's `retry_after`** | Slice 3, issue [#92](https://github.com/imonroe/brawling-mahogany/issues/92). `pending` plus a `message_key` does not mean the worker died — it means some worker claimed the row and has not written its outcome, which most often is a sibling delivery inside the provider call right now. Recording that as a failure put *"an automated message did not go out"* on the deal beside the *"Emailed …"* about the same message, and with the writes reversed left a delivered message marked failed, which invites the resend the claim exists to prevent. The window is read from the configured queue connection rather than written down, so a deployment that gives workers longer does not turn live sends into failures |
| 2026-08-26 | **A message a rail is holding appears on S47 as *Held*, not only as a number in settings** | Slice 3, issues [#93](https://github.com/imonroe/brawling-mahogany/issues/93), [#96](https://github.com/imonroe/brawling-mahogany/issues/96). Once the sweep stopped re-dispatching held messages every minute they became genuinely idle, and `pending` was on no list on the queue screen — so a team over its daily ceiling had client messages no screen named, indefinitely, while the manual pointed them there for *"what your automations are about to send"*. The row carries the rail's own reason, and the section says the messages go on their own once it clears |
