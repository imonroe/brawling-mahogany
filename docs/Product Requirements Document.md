...
| F2.2 | Access roles | Super Administrator, Team Owner, Team Member, Status Viewer, Contact. Assigned **per team**. Only Team Owner and Team Member put somebody *on the team* — and that follows from the **surface** of the permissions the role holds (§6.2), never from a list of role keys, so a custom role under F2.3 is covered by the same rule. | Must |
| F2.3 | Custom roles and permissions | Team owner composes roles from the available permission set, scoped to their team. A composed role holding any team-surface permission makes its holders team members everywhere the product asks — `/settings/members`, the People index's Team segment, the team switcher, and the console — so they can be revoked from the screen that revokes people. | Should |
...
| F5.9 | Send safety rails | Per-team rate limit, hard "no sends" switch, sandbox mode redirecting to the team owner. Asked at the send path, in the worker, immediately before the transport — a rail checked at dispatch is one a message queued five minutes before somebody pulled the cord sails straight past. Exceeding a limit **halts and alerts; it does not silently drop**, so the row stays scheduled and goes when the rolling window moves. | Must |
...
| `permissions` | key, group, description | Flat, seeded in code. Each key also carries a **surface** in code (`App\Enums\PermissionSurface`: team app, client status page, or platform console) — not a column, because `App\Support\Permissions` is the authority and the table is its output. The surface is what decides whether holding the permission means being on the team. |
...
| `action_instances` | deal_id, stage_id, action_definition_id, action_type, trigger, message_template_id, config JSON, state, scheduled_for, executed_at, payload JSON, approved_by, approved_at, message_key, provider_message_id, attempts, error |
...
**Where this actually stands (2026-08-28, #12).** SES is configured and `monroedigitalconsulting.com` is verified; every message leaves as `goldieflow@monroedigitalconsulting.com` over SMTP, and the account is in production access. Of the requirements above, **three are met — though one of those only in the application, with console setup still outstanding — and the rest are partly met, unconfirmed, or not built** — and the gaps are worth naming rather than leaving to be discovered, because most of them look identical from inside this application right up to the moment a client should have received something:

| Requirement | Status |
|---|---|
| Dedicated sending subdomain | **Not met.** Sending from an existing consulting domain instead, so the product's reputation is mixed with whatever else that domain sends. A deliberate trade to unblock Slice 3 rather than wait on #15's naming decision; revisit before the reputation is worth anything |
| SPF, DKIM, DMARC | **DKIM: likely.** SES verifies a domain through Easy DKIM CNAMEs, so `d=monroedigitalconsulting.com` signing is probably real. **SPF alignment and DMARC: unconfirmed, and neither follows from the other.** SPF is evaluated against the envelope MAIL FROM, and without a custom MAIL FROM domain in SES the Return-Path is `*.amazonses.com` — so the domain's own SPF record is never consulted and SPF is unaligned for DMARC, leaving DMARC to ride on DKIM alignment alone. DMARC itself is a `_dmarc` TXT record, entirely outside SES setup. #12's evidence says a domain is approved for sending and nothing about any of the three; check them in DNS **now**, because the first real send has already happened. SPF is very likely unaligned, and that on its own is **not** a DMARC failure — DMARC passes on DKIM alignment alone, which is why the sentence above says it rides on DKIM. What it means is that there is no second leg: DMARC holds only while Easy DKIM is genuinely signing, and is worth nothing at all if no `_dmarc` record is published. Neither has been checked, and mail is already going out |
| SES production access | **Met, 2026-08-28 (#12).** Kept here as history, because the distinction is the one to re-check for every new environment: a verified domain identity and a production account are different grants — in the sandbox an account may only send to addresses it has also verified, so a message to a real client is rejected rather than delivered, and the two are indistinguishable from inside the application. Production access is also granted **per region**. Note the consequence for staging, in §8.6 and #196: the sandbox was doing work, and it has stopped |
| SNS bounce and complaint webhooks | **Met in the application, 2026-08-27 (#95).** `POST /webhooks/ses` verifies Amazon's signature and the topic ARN, records `message_deliveries`, and writes `suppressed_addresses` automatically. What is outstanding is console-side and per environment: the SNS topic, its subscription, and `SES_SNS_TOPIC_ARN` — the endpoint refuses everything while that is empty, by design, so an unset ARN means bounces go unrecorded and suppression never fires |
| Per-team sending identity | **Met, in the three slots that carry it.** The address stays the one verified identity — SES rejects an unverified `From` at the API, and one the message is not DKIM-aligned with fails DMARC. The team's `sending_identity_email` goes in `Reply-To`, so a client's reply reaches the agency rather than the product's mailbox. And the team's **name** goes in the inbox line as *"Bosart Group via Goldieflow"*. All three in `App\Support\Mail\SendingIdentity`; shipping the first two without the third is the defect review caught, and it was worse than doing nothing |
| One-click unsubscribe | Not needed yet. Everything sent today is transactional; Slice 6's Keep in Touch inherits the layout with the block switched on |
...
Local Docker, a staging droplet mirroring production, and production. Staging **must** redirect all mail to one address via `MAIL_REDIRECT_TO`, so that no test reaches a real client — *must*, because nothing enforces it and an unset value fails open. **This was two guards and is now one:** the SES account reached production access on 2026-08-28 (#12), so SES's own sandbox no longer refuses unverified recipients at the API. The remaining guard is ours, it fails open when unset, and it is observable only from the droplet — see [[Deployment]] §4a and #196. F5.9's per-team `sandbox_mode` still redirects *automated* sends to the team owner, but it is per team and switchable, so it is not a substitute. Staging must also use a separate AI provider key with its own budget cap.
...
**Exit:** the contingency calendar drives real deadlines, and a real seller opens the status page on a phone and understands it without calling. **Built** (#105–#112). The half of that sentence a test cannot reach — a real seller, on a real phone — is the manual screen-reader pass #112 leaves open, and it is named in the pull request rather than counted as done.
...
| 2026-08-23 | **"Is this person on the team" is decided by the *surface* of a permission, not by counting permissions and not by a list of role keys** | Slice 2, issue [#142](https://github.com/imonroe/brawling-mahogany/issues/142). Slice 1 answered it two ways at once: `TeamMembership::carriesAccess()` and `Person::activeTeams()` asked whether any role carried any permission, while `/settings/members`, the People index's Team segment, and the console's team detail each named `['team_owner', 'team_member']`. The answers agreed only because the shipped roles happened to line up. **Two things were about to break it.** A team composing its own role (F2.3) gets somebody who can act in the team, appears on none of those three lists, and therefore cannot be revoked from the only screen that revokes people. And the moment #110 gives Status Viewer even one permission, "any permission at all" turns every client into a member — so removing a client from the directory starts routing through `RevokeMembership` and needing `team.members.manage`, which Heather does not have. **The decision.** Every permission in the catalogue declares a surface — team app, client status page, or platform console — and team access means holding at least one on the **team** surface. *Not* a `grants_team_access` flag on `roles`: `roles` is customer data under F2.3, a security-relevant flag there needs a default, and both defaults are wrong — `false` recreates exactly the bug above, `true` hands app access to the first client-facing role anybody builds. The permission catalogue is product data, finite and seeded in code, so classifying it is a decision the build can force and a customer cannot get wrong. Held by `tests/Isolation/TeamAccessConventionTest.php`: a role added without a recorded decision fails the build, as does a permission added without a surface, as does any new file deciding team membership by naming role keys |
| 2026-08-24 | **The person lifecycle is nullable, and a colleague holds null** (IA §8). `team_memberships.status` no longer has to hold a value; a screen draws the roles, the lifecycle, and *Revoked* as three independent facts, each when it is true | Slice 2, issue [#162](https://github.com/imonroe/brawling-mahogany/issues/162). `PersonLifecycleState`'s values are stages of a client relationship — Lead, Client, Past Client, Archived — and `AcceptInvitation` wrote `active` onto a colleague's membership because the column could not be empty. `active`'s label is **Client**, so the directory told a team that their own assistant was a client of theirs. **The first fix asked the wrong question.** Hiding the badge for anybody carrying team access works until that access is revoked, at which point the row falls back to the value nobody chose and the green *Client* returns — review measured it. *Was `active` typed or defaulted?* is unanswerable in a `NOT NULL` column, so the column changed instead: null means **no place on the client lifecycle**, which is what a colleague holds and what a former colleague holds until the team says what they are now. The same shape as the Vendor decision below: not a fifth lifecycle value, but a fact the schema was forcing into the wrong one. **`isColleague()` (access and not revoked) is what S32 refuses the field on and what the Leads and Clients segments filter by** — `scopeNotColleagues()` is that same question in SQL, because the badge and the segment beside it asked two different ones and a reclassified former colleague appeared on no segment but All. It is also what decides whether accepting an invitation clears the lifecycle — asked of who is on the team **after** the accept, not of the invited role, because a live membership's roles are a union and a Team Member given a status page is still a Team Member. Anybody else keeps the lifecycle they had, or gets `active` when there is none to keep, which preserves what the product did before and leaves the `Contact` role badged *Client* when it may be a lender or an inspector. That guess exists because the People row draws a role badge only for a **team**-access role, so a professional contact's row would otherwise be blank — and the fix is that badge rather than a fifth lifecycle state, since `ParticipantRole` and `vendor_specialties` already name a lender, an inspector, and a painter. [#167](https://github.com/imonroe/brawling-mahogany/issues/167) |
...
| 2026-08-25 | **`Email Template` becomes `Message Template`, in the table, the model and the route** — `/templates/messages`, not `/templates/emails` (§7.12) | Slice 3, issue [#90](https://github.com/imonroe/brawling-mahogany/issues/90). §7.12 already said the concept should generalise and the v0.2 update put `push` beside `email` and the deferred `sms` in the channel enum; the rename is what makes the code say it. A route named after one channel is wrong the first time somebody writes a push template, and Screen Inventory S45 and S46 moved with it |
| 2026-08-25 | **`message_templates` is fully team-scoped, unlike the rest of the definition layer** | Slice 3, issue [#90](https://github.com/imonroe/brawling-mahogany/issues/90). `workflow_templates` and its children take a nullable `team_id` so a pack can ship one shared row to every team, and that is safe because none of them holds customer data — a stage template says *"Listing Preparation, 5 days, owned by the transaction coordinator"* and names nobody. A message template is **the team's own words to their own clients**, with their signature under it and their sending identity on it, which is the closest thing in the definition layer to customer data. So it gets all five ADR 0002 layers rather than a named scope, and a pack that eventually wants to ship an emailing automation **copies** its template into the team the way `CopyTemplate` already copies a workflow template. The consequence is enforced rather than remembered: `action_definitions` carries a composite foreign key **and** a CHECK constraint refusing a shared row a template pointer at all, because a Postgres foreign key is MATCH SIMPLE and a null `team_id` satisfies one without checking anything |
| 2026-08-25 | **F5.4's manual prompt and F5.7's approval are one three-way choice, and the pair is mutually exclusive in the database** | Slice 3, issue [#91](https://github.com/imonroe/brawling-mahogany/issues/91). Both put a human in the loop and they describe the same moment from two ends, so an automation carrying both would ask two people to agree to one email. `action_definitions` keeps the two columns §6.2 names and refuses the combination with a CHECK, so a route added in a later slice inherits the invariant; S44 offers *fires on its own · needs approving first · prompts somebody to do it*. This is the Screen Inventory's *"a progressive form that narrows, not four independent dropdowns that can be combined into nonsense"*, made structural |
| 2026-08-25 | **Two of F5.6's merge fields are registered and cannot resolve, rather than omitted** | Slice 3, issue [#90](https://github.com/imonroe/brawling-mahogany/issues/90). Key dates arrive in Slice 4 (#109) and the status page link in Slice 4 (#110). Each is in the registry carrying the slice that wires it, so the editor names it and the save-time validator refuses it **by name** — *"there is no such field"* would send somebody looking for a spelling mistake. Same shape as `GateRegistry::selectableOptions()`, and the same reason: a picker that offers what cannot work builds something that silently never fires |
| 2026-08-25 | **`{{ stage }}` resolves to the client-facing label and the internal stage name is not offered as a merge field at all** (IA §9) | Slice 3, issue [#90](https://github.com/imonroe/brawling-mahogany/issues/90). F5.6 lists *stage* among the merge fields and IA §9 is absolute that a client never sees an internal stage name — they say things like *"Chase lender"*. A message template is a client-communication instrument, so there is one field and it is the safe one. A field that exists is a field somebody will use; an internal channel that needs the internal name can have its own the day something needs one |
| 2026-08-26 | **Idempotency is two columns: `message_key` (ours, written before the mailer) and `provider_message_id` (theirs, written after they answer)** | Slice 3, issue [#92](https://github.com/imonroe/brawling-mahogany/issues/92). A provider call can time out *after* the provider accepted the message, so an id handed back by the provider is exactly what a timed-out send does not have. The key is claimed with a conditional `UPDATE … WHERE message_key IS NULL`, which makes the database decide which of two workers owns the send. The crash window that leaves is the safe one: a `pending` row carrying a key, which every path **stands down on** and `automations:reap-unconfirmed` settles later. The other ordering leaves a row that looks unsent and is not |
| 2026-08-26 | **An automation's instances are raised inside the advance's transaction; the queue dispatch happens after it commits** | Slice 3, issue [#92](https://github.com/imonroe/brawling-mahogany/issues/92). Both halves are load-bearing and they pull in opposite directions. A row written outside the transaction is a message in a team's approval queue for an advance that rolled back; a job dispatched inside it can be picked up by a worker before the commit lands, or after a rollback the queue never hears about. `AdvanceWorkflow` has named this seam since Slice 2 |
| 2026-08-26 | **A stage's automations are snapshotted with the rest of the process; the message template pointer stays live** | Slice 3, issue [#92](https://github.com/imonroe/brawling-mahogany/issues/92). F4.5 says a template edit never rewrites a running deal, and an automation is as much part of a process as a gate is — a team that adds *"email the seller on completion"* today does not retroactively owe an email on every stage that completed last month. The **words** are the exception, deliberately: they are rendered when an instance is raised, so a typo fixed in a template reaches messages raised after the fix and not ones already queued. Fixing a typo should not require rebuilding a workflow |
| 2026-08-26 | **An automation that already fired on a stage does not fire again when that stage is reopened — unless the first instance was cancelled** | Slice 3, issue [#92](https://github.com/imonroe/brawling-mahogany/issues/92). `AdvanceWorkflow::reopen()` wrote the contract before there was a table to hold it. The subtlety is telling *"already said"* from *"never said"*: a skipped stage cancels its queue and nothing went out, so a stage that comes back and is worked properly is still owed its message. Excluding cancelled rows is what makes both true; a dedupe over every row silences the second case forever |
| 2026-08-26 | **F5.7's approval window is 30 days from now for every existing team, and new teams start in sandbox mode** | Slice 3, issues [#93](https://github.com/imonroe/brawling-mahogany/issues/93), [#96](https://github.com/imonroe/brawling-mahogany/issues/96). Two defaults chosen on the safe side rather than the convenient one, and both are a team setting either can turn off. The window is *"the safety net for exactly the period when a team's templates are least tested and their trust is most fragile"*; measuring it from now rather than from signup means a team that has been running since before this shipped gets the net too, which is right the first time an automation they wrote months ago finally fires |
| 2026-08-26 | **There is no bulk approve, and no route that could become one** | Slice 3, issue [#93](https://github.com/imonroe/brawling-mahogany/issues/93). The issue names the failure mode: *"bulk approve teaches people to approve without reading."* Every release is one message, opened, with its words on screen. A test asserts the route list rather than the screen, because a front end that chose not to draw the button would still leave the endpoint there |
| 2026-08-26 | **An approver may edit a queued message, and may not type a merge field into it** | Slice 3, issue [#93](https://github.com/imonroe/brawling-mahogany/issues/93). The payload is text that has *already been substituted* — the render happens at raise time so F5.10 can pre-fill it — so a `{{ closing_date }}` typed into the editor has nothing left to replace it and reaches the client as braces. Registered or not is beside the point. Refused rather than substituted, because substituting would fix the values at approval time on one message out of two raised from the same words |
| 2026-08-26 | **`message_templates.from_identity` is a reply-to, not a from** | Slice 3, issue [#92](https://github.com/imonroe/brawling-mahogany/issues/92). An outbound address has to be one SES is authorised to send as, or the call is rejected outright — and one the message is not DKIM-aligned with fails DMARC and lands in spam, which for this product is the same failure as not sending it, except that nobody finds out. (The mechanism as first written here said SPF, which is evaluated against the envelope MAIL FROM rather than the `From` header; corrected with #12.) The envelope's from stays the application's verified identity; #94 is where a team gets one of its own |
| 2026-08-26 | **F5.7's review window opens on team creation, on the model rather than in `ProvisionTeam`, and a team can end it from `/settings/sending`** | Slice 3, issues [#93](https://github.com/imonroe/brawling-mahogany/issues/93), [#96](https://github.com/imonroe/brawling-mahogany/issues/96). Round 1 of review found the column was set by the migration's backfill and by nothing else — so the rail §4.5 calls a launch blocker was live for every team that already existed and dead for every team it was written for. `ProvisionTeam` is not the only door (the console provisions teams, and signup will), so the rule lives where every caller passes. And it is a control rather than a notice: a safety default a team cannot leave is not a default, and a team that cannot leave it will find a way around it that nobody audits |
| 2026-08-26 | **A send path refusal distinguishes "this message is broken" from "this row is not mine"** | Slice 3, issue [#92](https://github.com/imonroe/brawling-mahogany/issues/92). `ApproveMessage::cancel()` allows stopping a **pending** instance, and a pending instance has already been dispatched — so *"queued → somebody presses Stop → the worker arrives"* is the ordinary sequence. Marking it failed destroyed the reason a person typed, contradicted itself on the deal's timeline, and flipped the row from `cancelled` to `failed` where the raise-time dedupe counts it, so a skipped stage that was later reopened silently never re-raised its message. `SendDecision::standDown()` writes nothing at all |
| 2026-08-26 | **F5.2's *when a requirement clears* fires for every gate type, raised where `is_met` transitions rather than where a person ticks** | Slice 3, issue [#92](https://github.com/imonroe/brawling-mahogany/issues/92). It was wired only to the manual-confirmation route while S44 offered the trigger for all seven types, so *"when the required tasks are done, email the buyer"* saved cleanly, showed as active, and never fired. False→true only; an **override** raises nothing, and the load-bearing reason is not that IA §8 keeps overridden and met apart — it is that **no evaluator reads `gates.overridden`**, so an overridden gate's verdict stays unmet and never transitions at all. **Noticed on the next Advance press rather than the instant the world changes**, since nothing re-evaluates a gate on task completion; `resources/help/automation.md` says so rather than letting the trigger's name imply otherwise, and evaluating from `DealTasks::complete()` is the follow-up that would make it literal |
| 2026-08-26 | **F5.9's rate ceiling counts outbound emails only** | Slice 3, issue [#96](https://github.com/imonroe/brawling-mahogany/issues/96). The ceiling exists so a looping automation cannot reach clients four hundred times; a created task reaches nobody. Counting every executed instance meant three `create_task` automations across a busy morning silently paused a team's actual client email — the rail causing the failure it exists to prevent |
| 2026-08-27 | **A derived date is calculated on save and stored, not computed on read** | Slice 4, issue [#106](https://github.com/imonroe/brawling-mahogany/issues/106). `key_dates.date` holds a real day for every row, derived or not. Computing on read is tempting — one source of truth, no cascade to write — and it makes the two things this feature exists for impossible: a reminder sweep cannot query for *the dates seven days out* if the dates are a graph walk, and an extracted date somebody confirmed in March cannot keep its day when the anchor moves in April. Storing means the cascade is a write, and a write is a thing that can be **previewed, audited, and refused** |
| 2026-08-27 | **A derived date whose anchor is deleted keeps its day and detaches; it never vanishes and never recalculates from nothing** | Slice 4, issue [#106](https://github.com/imonroe/brawling-mahogany/issues/106). The self-referencing foreign key is `ON DELETE SET NULL` with the column list named explicitly, because a composite `SET NULL` blanks **every** referencing column unless it is told otherwise — which would have taken `team_id` with it. The product consequence is the point: *inspection deadline* was calculated from acceptance, and deleting the acceptance date does not make the inspection deadline stop existing. `detached_at` records that it used to be derived, so the screen can say *"this no longer follows anything"* rather than presenting a typed date it did not type |
| 2026-08-27 | **Business days skip weekends and nothing else — there is no holiday calendar, and the limit is stated rather than implied** | Slice 4, issue [#106](https://github.com/imonroe/brawling-mahogany/issues/106). A holiday calendar is jurisdictional, it changes annually, and a wrong one is worse than none because somebody trusts it. `OffsetBasis::phrase()` says *business days (weekends skipped)* wherever the choice is offered, and `resources/help/dates-and-deadlines.md` says it in the user's words. The alternative was quietly meaning *weekdays* while writing *business days*, which is the same date arithmetic and a different promise |
| 2026-08-27 | **Recurrence is expanded on read and never stored as occurrence rows** | Slice 4, issue [#105](https://github.com/imonroe/brawling-mahogany/issues/105). Fifty-two rows for a weekly showing means an edit has to find and rewrite fifty-two of them, and the occurrence somebody's calendar subscribed to becomes a row rather than a rule. `Recurrence::occurrencesBetween()` skips to the first step near the requested window in closed form, so a daily event five years old costs what a new one costs, and caps at 1,000 occurrences so a malformed rule cannot spin a request |
| 2026-08-27 | **A critical date falling *today* is the only notification in the product that bypasses quiet hours** | Slice 4, issue [#109](https://github.com/imonroe/brawling-mahogany/issues/109). F12.4's rule is about being **woken**, and everything else this product raises can wait until seven. A financing contingency expiring today cannot: the whole reason the date is marked critical is that missing it has a consequence money cannot undo. `NotificationType::bypassesQuietHours()` returns true for exactly one case, so the exception is a value in an enum somebody has to add themselves rather than a flag a caller can pass |
| 2026-08-27 | **A status page link is two credentials in one row — a 30-minute single-use link and a 14-day session — and the link is never the session** | Slice 4, issue [#110](https://github.com/imonroe/brawling-mahogany/issues/110). A magic link that *is* the session is a credential sitting in an inbox, forwarded, and quoted in a reply chain for as long as the deal runs. Redeeming exchanges it for a session token in a conditional `UPDATE`, which also makes *already used* a state the database decides rather than a race two tabs can win. Fourteen days because a seller checks in roughly weekly for three months and re-sending a link every visit is how a client stops looking; thirty minutes because the link is the half that travels |
| 2026-08-27 | **A stage with no client-facing announcement is omitted from the client timeline rather than shown under its internal name** | Slice 4, issue [#111](https://github.com/imonroe/brawling-mahogany/issues/111). IA §9 is absolute that a client never reads an internal stage name, and the only two ways to honour it are to omit the stage or to invent a label. Inventing one puts words in the team's mouth on the team's own branded page. So `ClientStatus` reads `stages.milestone_label` and nothing else, and a workflow whose stages carry no labels renders a status card and no timeline — which is a legible page saying little, not a broken one. It also means the language rules are enforced **on the server**: a template holding the rows could reach for `stage.name` in one place and forget, and the failure is silent until a seller reads *"Chase lender"* |
| 2026-08-27 | **An iCal feed URL is a bearer token, so the feed carries the minimum that makes it useful and nothing more** | Slice 4, issue [#108](https://github.com/imonroe/brawling-mahogany/issues/108). A subscription URL ends up pasted into Google Calendar, which fetches it forever from an address nobody can revoke by changing a password. So there is no `ATTENDEE` (a colleague's address in a document a third party stores), no `DESCRIPTION` (free text that has carried a client's name since the first note somebody typed), and the response is `private, no-store` and `noindex`. Revoking is the only control that matters, so an unknown token and a revoked one both 404 — the difference between them is exactly what confirms a token was once real |
| 2026-08-28 | **`{{ next_deadline }}` is wired and resolves to the *day only*; `{{ status_page_link }}` stays registered-but-unavailable, for a new reason** | Slice 4, issues [#109](https://github.com/imonroe/brawling-mahogany/issues/109), [#110](https://github.com/imonroe/brawling-mahogany/issues/110), [#111](https://github.com/imonroe/brawling-mahogany/issues/111). The 2026-08-25 entry below registered both fields carrying the slice that would wire them, and this is that slice reporting back on each. **`next_deadline`** resolves to the soonest confirmed date at or after today — the same predicate `ClientStatus` reads, so an email and the page it links to cannot name different days — and to the **day and never the date's own name**, because a key date's name is free text a team typed for their own screens and IA §9 does not let a merge field be the way internal wording reaches a client. That is the `{{ stage }}` argument one table over. **`status_page_link`** did not get wired, and the note on it now says why rather than naming a slice that has landed: what is missing is a *credential a message may carry*. `IssueStatusPageLink::issue()` mints a 30-minute single-use link and revokes every live grant that person holds on the deal — right for a client asking for a new link, wrong for an automation, which would end the session of a client browsing the page at the moment a stage completed. Pointing at the session they already hold is not available either, because the session token is stored hashed by design. Access to the page is a deliberate act on the deal's People tab until that is settled; S87's frame keeps the slot, empty. |
| 2026-08-28 | **A feed URL is checked against `calendar.view` on every fetch, and a role edit gates it rather than revoking it** | Slice 4, issue [#194](https://github.com/imonroe/brawling-mahogany/issues/194). The 2026-08-27 entry below settled what a feed may *carry*; this settles who may still *fetch* one. `findByToken()` asked whether the person had left the team and never whether they could still open the calendar, so a broker moved onto a narrower composed role — two clicks on S75 — met a 403 on S57 and kept a live `.ics` in Google delivering every showing and closing date the team has. The key is the screen's own, `calendar.view`, asked of the membership in the **feed's own team**. Not `carryingAccess()`, which is **wider**, not stricter: `calendar.view` is itself a Team-surface permission, so anybody this predicate admits `carryingAccess()` admits too, and it additionally admits the person holding only `deals.view` — exactly the case the fix exists to stop. (The *"reads more strictly than the app writes"* argument in #193's review was made against a **revocation-only** baseline, where `carryingAccess()` is the stricter of the two; it does not survive being quoted against a candidate stricter than both.) And not a new key, which no shipped role would hold. It **gates**: nothing is written, so restoring the role restores a subscription already sitting in somebody's calendar, and a role edited by mistake costs a fetch interval rather than every URL the team has issued. The cost of choosing gate over revoke is that while the gate is closed nothing reaches Revoke: the holder is refused by `destroy()`, which authorises on `calendar.view` before it asks whose feed it is, and an owner holds the permission but has no screen naming the row, because `feedsFor()` lists a person their own feeds only. So a URL in a third party's calendar is re-armed by a later role edit rather than ended by one — #206 |
| 2026-08-28 | **#112's AA claim is bounded to the accents S72 does not warn about, and the warned band gets a floor rather than a promise** | Slice 4, issue [#112](https://github.com/imonroe/brawling-mahogany/issues/112). A team can pick any hex, and the mid greys around `#777777` reach 4.478:1 against white and less against near-black — there is no foreground that makes them AA for body text. The choices were to reject those colours, to silently override them, or to say what is true: outside the band S72 warns about the client surface computes a foreground that clears 4.5:1, and inside it the page takes the better of the two and never drops below 3:1. `ClientContrastTest` asserts both halves, including the bound, so the claim cannot quietly widen |
| 2026-08-28 | **Staging's mail safety net was two guards and is now one, and the survivor fails open** | Slice 3, issues [#12](https://github.com/imonroe/brawling-mahogany/issues/12) and [#196](https://github.com/imonroe/brawling-mahogany/issues/196). §8.6 used to read *"Staging runs SES in sandbox mode with all mail redirected"*, which was **two** guards, and the outer one was free: the SES sandbox refused unverified recipients **at the API, outside this application**, so it held even when the application was misconfigured. Production access removed it. What is left is `MAIL_REDIRECT_TO`, and the asymmetry is the part to remember — production with the redirect *set* (every client update silently stops) throws at boot, while staging with it *unset* (mail reaches real people) returns early and says nothing. F5.9's per-team `sandbox_mode` is **not** a substitute: it is per team, switchable from `/settings/sending`, and covers only automated sends, so the invitation, the password reset and the client's own status-page link go nowhere near it. Recorded here rather than left to §8.6 because the guarantee a document states is the thing people plan around |
| 2026-08-26 | **A worker never decides the outcome of a claim it does not own; `automations:reap-unconfirmed` does, hours later** | Slice 3, issue [#92](https://github.com/imonroe/brawling-mahogany/issues/92). `pending` plus a `message_key` does not mean the worker died — it means *some* worker claimed the row and has not written its outcome, most often a sibling inside the provider call at that instant. Three rounds of review went into trying to tell those apart inside the next worker and it cannot be done: the second delivery happens **because** the claim aged past the queue's visibility timeout, so both shapes are the same age at the only moment a worker looks. A threshold below it narrates failures about live sends; one above it is never evaluated, because standing down completes the job and there is no third delivery. Distance is the discriminator, so the outcome is recorded by a scheduled sweep far from the claim — and in the meantime the row is **visible on S47**, which is a read and cannot contradict anybody |
| 2026-08-26 | **A message a rail is holding, or a claim nobody confirmed, appears on S47 as *Held*** | Slice 3, issues [#93](https://github.com/imonroe/brawling-mahogany/issues/93), [#96](https://github.com/imonroe/brawling-mahogany/issues/96). Once the sweep stopped re-dispatching held messages every minute they became genuinely idle, and `pending` was on no list on the queue screen — so a team over its daily ceiling had client messages no screen named, indefinitely, while the manual pointed them there for *"what your automations are about to send"*. The two kinds are told apart in the copy, because only one of them is true of both: a rail's hold clears and the message goes on its own, and an unconfirmed claim never will |
| 2026-08-26 | **One email layout, and every `Mailable` in `app/Mail` extends it** | Slice 3, issue [#97](https://github.com/imonroe/brawling-mahogany/issues/97). Narrow on purpose: Fortify's password reset is a framework notification rendering Laravel's own markdown mail, it goes to a colleague rather than a client, and bringing it into the frame means replacing that notification — a separate change. Three views had their own frame and had already drifted apart — two card widths, two greys, one footer between them — which is Design System §12's argument for having a layout, arriving on schedule. `mail.layout` implements §12's ten rules and `App\Support\Mail\EmailPalette` carries §12.1's hex, held against the document by a test the way the enums are held against these tables |
| 2026-08-26 | **Design System §12.1 gains a dark column, which is not §2.6's deferral being walked back** | Slice 3, issue [#97](https://github.com/imonroe/brawling-mahogany/issues/97). §2.6 defers dark mode *for the app*, where the product decides when a theme applies. An email has no such control: iOS Mail and Outlook.com invert a message the day the reader turns dark mode on, designed for or not. §12 already required `prefers-color-scheme` and §12.1 had no values to do it with. The `!important` on every rule in that block is load-bearing — each one overrides an inline style, so a dark block without it parses cleanly, does nothing, and looks handled |
| 2026-08-26 | **In email a team's accent is a fill with a computed foreground, never text — narrower than §2.7** | Slice 3, issue [#97](https://github.com/imonroe/brawling-mahogany/issues/97). §2.7 gives the accent to headings, markers and links, which is safe on a surface the product renders in a theme it chose. A reader in dark mode gets `#1A588F` on near-black, and the darker a team's brand the less of their own heading they can read — and a team picks a deep colour *because* it looks right on white. So the accent appears only as the header band and the call-to-action button, both of which carry their own ground. The foreground is computed with `AccentContrast`: S72 **warns** an owner about a low-contrast accent and takes it anyway, which is right there, and an email has no second chance and nobody standing in front of it |
| 2026-08-26 | **A team's logo is embedded in the message, from a private disk, on a plate that never inverts** | Slice 3, issue [#97](https://github.com/imonroe/brawling-mahogany/issues/97). Linked rather than embedded would mean a public URL for a tenant asset, and a client reading the email has no session to fetch an authorized one with — a broken image for the one reader it is for. The plate is §2.6's `--logo-plate` one universe over: *"a raster asset cannot participate in the token layer"*, so a mark drawn for white gets white behind it in both schemes. This is also where `teams.logo_path` finally got a **writer**: #55 shipped the column, S72 rendered it, and nothing could set it — a reader with no writer is as dead as a row nothing can reach, and reads as finished from either end |
| 2026-08-26 | **S87 is a frame around an ordinary automated message, not a second mailable** | Slice 3, issue [#97](https://github.com/imonroe/brawling-mahogany/issues/97). A `MilestoneNotificationMail` of its own would be a second path to a client's inbox, past F5.7's queue and F5.9's rails — a second front door into the highest-blast-radius feature in the product (§4.5), for the sake of a layout. So a milestone notification is a `stage_completion` automation like any other, wearing a frame that opens with the stage's `milestone_label` (IA §9, never the internal name). Only a **completion** announces one: a milestone is *the notable completion of a stage*, and a `stage_start` email would open with "Your home is on the market" the morning the photographer was booked. `Stage::clientAnnouncement()` had carried that argument since Slice 2 with no caller at all |
| 2026-08-26 | **The frame offers the listing only when the team's own words do not already carry it** | Slice 3, issue [#97](https://github.com/imonroe/brawling-mahogany/issues/97). `{{ mls_link }}` is a registered merge field and §5.4's worked example is exactly that email, so a frame that added its own button regardless would send half the teams in the product a message with the same URL in it twice — once as they wrote it and once as we decided. Checked against the **rendered** body rather than the template's tokens, because an author can write the URL out in full; and against both spellings of it, because `RenderMessage` escapes into `body_html` and leaves `body_text` alone, so a query string is `&amp;` in one half and `&` in the other |
| 2026-08-26 | **S87's announcement is snapshotted into the payload at raise time, and the team's branding is not** | Slice 3, issue [#97](https://github.com/imonroe/brawling-mahogany/issues/97). Resolving the announcement in the mailable would put a **live** read of the deal in the same email as a body rendered when the automation fired — the address in the header and the address in the paragraph under it, answering one question from two moments. The sharper reason is F5.7: a message can wait days for approval, and what an approver reads on S48 *is* the payload, so anything in the email not derived from it was approved by nobody. Branding is deliberately the opposite — a logo and an accent are the team's own identity, the same on every message, and a message held for two days should go out wearing the logo the team has now. An instance raised before this shipped has no `milestone` key and renders a plain frame, which needs no migration |
| 2026-08-26 | **S91 is a scheduled sweep that reads `state`, not a hook on the failure** | Slice 3, issue [#97](https://github.com/imonroe/brawling-mahogany/issues/97). A failed message is already a red row on S47 and an entry on the deal's timeline, and both are **pull** — true the moment somebody looks, and nobody looks at a queue that is usually empty. The failure this closes is a team believing a client was told, for a fortnight. The first implementation hung off `ExecuteAction::fail()` and adversarial review found it **never fired for the outage it was written about**: a transport exception is caught in `send()`, recorded and re-thrown, and never reaches `fail()`. Every test was green because every test drove the same rail refusal. That is the `gate_cleared` finding one feature over — a thing wired to one implementation of a failure is wired to none of it — so `automations:alert-on-failures` asks the table instead, where a row is `failed` however it got there. A **halt** is excluded deliberately: a halted message is still `pending`, nothing is lost, and both surfaces for it say so in the present tense |
| 2026-08-26 | **The alert's throttle is a high-water mark, and its count is taken after the burst rather than during it** | Slice 3, issue [#97](https://github.com/imonroe/brawling-mahogany/issues/97). Two consequences of the same move. Fired on the failure, the alert went out **first** — the moment the backlog is at its smallest — so forty dead messages produced one email reporting one; the number did not exist at the only instant anything was running, which is the unconfirmed-send finding again. And an hour-long throttle re-alerts about the same standing backlog every time it expires, forever, until somebody clears rows nobody may ever clear; a mark that only ever speaks about failures nobody has been told about does not nag. It also **cannot throw**: a sweep that died on the first broken team would never reach the second, and it runs for every team on the platform |
| 2026-08-26 | **The mark is a column on `teams`, and the window it defines is half-open** | Slice 3, issue [#97](https://github.com/imonroe/brawling-mahogany/issues/97). Two findings from round 2 of review, both invisible to a green suite. `action_instances.executed_at` is `timestamp(0)`, so a burst puts many failures in one second — and a mark set to a reported row's own timestamp, read back with a strict `>`, silences every sibling that landed in that same second after the sweep's `SELECT`, permanently. The sweep picks its own boundary now — at this point the start of the current second, moved behind the sweep by the entry below — and reports `[mark, boundary)`, so every instant belongs to exactly one window. **A frozen clock cannot see that defect**, which is how it survived a round: every test made its failures and swept inside one second, the one arrangement the scheduler never produces. And the mark itself moved from the cache to a column: Redis is evictable and empty after a restart, while `automation.md` promises a team *"never twice about the same failure"* in those words — and a mark that had never been written fell back to a cold-start floor relative to `now()`, which slides forward, silencing the backlog the no-audience branch claimed to be preserving |
| 2026-08-26 | **The window's boundary sits a minute behind the sweep, and keys on `executed_at` alone** | Slice 3, issue [#97](https://github.com/imonroe/brawling-mahogany/issues/97). Round 3 of review, and both halves are the previous round's fix being one assumption short. A half-open window fixes *which* second a row belongs to and still assumes a row stamped below the boundary is visible to a query taken at it — but `executed_at` is stamped in PHP by whichever process wrote the row and becomes visible at **COMMIT**, and `onOneServer` pins the scheduler rather than the writers, so a worker a second slow backdates everything it writes. Distance is the answer again, a minute rather than the reaper's six hours. Gross clock skew still defeats it and the code says so rather than implying otherwise. And `COALESCE(executed_at, updated_at)`, added to make *"a row is failed however it got there"* true of the column the sweep reads, let `updated_at` drag a row in front of the mark on every save — an email about the same row every five minutes, against the promise the help article makes in the same words. The hole it covered is unreachable, since the only writer of `failed` sets both columns in one statement, so for a row made by hand silence is the better of the two wrong answers |
| 2026-08-26 | **The verified sending address is the product's; the team gets the display name and the reply-to** | Issue [#12](https://github.com/imonroe/brawling-mahogany/issues/12). SES is configured against `monroedigitalconsulting.com`, so every message leaves as `goldieflow@monroedigitalconsulting.com`. §8.5's *"per-team sending identity"* is then a question about **which slot**: the address cannot move, because SES rejects an unverified `From` at the API and a `From` the message is not DKIM-aligned with fails DMARC — the same failure as not sending, except nobody finds out. (Not SPF, which is evaluated against the envelope MAIL FROM rather than the header; the mechanism is worth getting right, because a wrong reason is how somebody later concludes the rule is negotiable.) So the team's own `sending_identity_email` rides in `Reply-To` — the field S72 has labelled *"Reply-to address"* since Slice 1 and which reached no header until now — and the team's **name** goes in the inbox line as *"Bosart Group via Goldieflow"* — the agency first because that is who the client thinks is writing, the product after because that is what makes an unfamiliar sending address read as explained rather than forged. **All three move together or not at all**: a From line naming the agency over a reply that lands in the product's mailbox is worse than leaving the product's own name there, because the old line at least matched where replies went. The per-team **branding** was never affected by any of this: a logo and an accent live in the message body, and the sending identity lives in the envelope |
| 2026-08-26 | **The product's name is its own config key, and the sending identity is one object with three slots** | Issue [#12](https://github.com/imonroe/brawling-mahogany/issues/12), round 2 of review. Two findings with one shape. `APP_NAME` is slugged into the session cookie and the cache, Redis and Horizon prefixes, so it is an **infrastructure identifier** and the rename note deliberately leaves it at the `Brawling Mahogany` codename — which meant every client-facing `From` line read *"Bosart Group via Brawling Mahogany"*, and both tests covering it asserted `config('app.name')` and so were true of any value. `APP_PRODUCT_NAME` is now the name a person reads and the only one safe to change; the rule is to check what else derives from a config value before pinning a display string to it. And the reply chain shipped in round 1 was empty for **every team that exists**: `sending_identity_email` is nullable and neither the migration, `ProvisionTeam` nor the factory sets it, so the ordinary case was still the blocker round 1 found. A team owner is the last link, and `SendingIdentity` returns the `From` and the `Reply-To` from one call — with no reply address it **drops the team's name** rather than naming an agency nobody can answer for. The invariant is structural instead of remembered |
| 2026-08-26 | **A display default written in two files was corrected in one, and a revoked owner is not a reply address** | Issue [#12](https://github.com/imonroe/brawling-mahogany/issues/12), round 3 of review. Splitting `APP_PRODUCT_NAME` out of `APP_NAME` fixed `.env.example` and `config/app.php` and left `config/mail.php` still reading `env('MAIL_FROM_NAME', env('APP_NAME', …))` — so every message the product itself writes (S91's alert, a template test send, a password reset) went on signing as the codename, and both tests covering it asserted `config('mail.from.name')` and so passed. `phpunit.xml` pins the value now and `ProductNameSeparationTest` reads the files instead, because a pinned test environment is exactly what a rendered-message test cannot see through. The provisioning script's managed block carries all three display keys, since the `.env` stage copies the example only when the file is **absent** — an environment provisioned before the split would otherwise never gain it. And `ownerReply()` had no `active()`: `oldest('id')` picks the *founding* membership, so a client's reply went to whoever set the team up however long ago they left, while F5.9's sandbox rail — which does filter revocation — redirected to somebody else entirely |
| 2026-08-26 | **The document guardrails ship before the upload button, and a scanner that cannot read says so** | Slice 3, issues [#98](https://github.com/imonroe/brawling-mahogany/issues/98)–[#100](https://github.com/imonroe/brawling-mahogany/issues/100), [#104](https://github.com/imonroe/brawling-mahogany/issues/104). §14.3 calls uploaded financial instruments the largest liability in the product, and Slice 2 avoided it by never building a general upload — #63 restricted the *context* instead, images only against a property only, because *"a photographed check is an image, exactly what a photo gallery accepts."* This slice reads the bytes, so the context restriction could be lifted. Three decisions carry the weight. `ReadableText::from()` returns **null** rather than `''` for an image or a text-free PDF, so `clean` and `not_scanned` stay different facts and no screen draws the first over the second — a badge reading "clean" over a photograph of a cheque would be believed. A refusal carries what, why, and **where to put it instead**, in a dialog rather than a field error, because #99 names the third as what makes it *"acceptable rather than infuriating"*. And the check is described to users as a backstop rather than a guarantee, since a photograph of a cheque has no text to read — which is exactly why F6.6's warning is a panel above the control and not a line of description |
| 2026-08-26 | **A document's permissions follow what it is attached to** | Slice 3, issue [#98](https://github.com/imonroe/brawling-mahogany/issues/98). `DocumentPolicy` keyed everything on `properties.*`, which was true while Slice 2's only documents were a property's photographs. S21 attached them to deals and the two halves stopped agreeing: the tab authorizes `view` on the **deal**, so a role holding `deals.view` without `properties.view` got a list of documents that then refused to download. No shipped role can reach it — Team Member holds both — but S75 lets a team compose its own, and a permission pair nobody can currently separate is not one nobody ever will. The match keys are **class names**: no morph map is enforced, so a string literal would match nothing and fail silently open to the default, which is the shape of the bug it fixes. `viewAny` deliberately takes the *wider* of the two, because S50 shows both kinds and authorizes each row on its way out |
| 2026-08-26 | **`config('app.name')` is a display value with no infrastructure reader, so it is the product's name too** | Issue [#12](https://github.com/imonroe/brawling-mahogany/issues/12), round 4 of review. Round 2 added `app.product_name` *beside* `app.name` on the premise that the latter was load-bearing — but every prefix that makes it load-bearing calls `env('APP_NAME')` **directly** (`config/session.php`, `config/cache.php`, `config/database.php`, `config/horizon.php`), so the config key had only display readers, and most of them are vendor views this application cannot edit. Fortify's password-reset email rendered the pre-rename codename four times and the product's name zero. Both config keys now resolve to `APP_PRODUCT_NAME`; the env var is untouched and the session cookie and three prefixes never move. The rule: **before adding a key beside an existing one, check whether the existing one can simply be corrected** — the guard test asserts the prefixes still derive from the env var directly, because that is what makes this safe |
| 2026-08-26 | **The test-environment pin lives in `TestCase`, because `phpunit.xml` could not pin** | Issue [#12](https://github.com/imonroe/brawling-mahogany/issues/12), round 4. PHPUnit writes an `<env>` with `putenv()`/`$_ENV` and **skips a name already present in `getenv()`**; Laravel's `Env` reads `$_SERVER` first; and `compose.yaml`'s `env_file: .env` puts every key of a developer's `.env` into the environment `make check` runs in. So the pin added in round 3 was green in CI, which copies `.env.example`, and ten tests red for every developer whose `.env` predated the split — which is all of them, since `make setup` never rewrites an existing one. Setting the config in `setUp()` involves no precedence rule at all, and a test asserts the pin is in force: a pin with no test is a hope, which is exactly what the first one turned out to be |
| 2026-08-26 | **A platform-provisioned invitation is from the product, and never carries the operator's sign-in address** | Issue [#12](https://github.com/imonroe/brawling-mahogany/issues/12), round 4. §5.1 step 1 — an operator provisions a team and invites its first owner — reaches `TeamInvitationMail` with an inviter who has **no membership in a team created four lines earlier**, and a team with no owner yet, so every link of the reply chain is empty and the From signs as the product. That is the right answer rather than a regression: the recipient is being invited to become the first member of a team that has none, a reply with no `Reply-To` still reaches the product's own verified mailbox, and ADR 0003 means the console hands the operator the accept link regardless. What *was* wrong is the body: `Person::displayNameWithin()` ends at the sign-in address, and its own docblock argues that is not a disclosure — for an **audit entry**, read by people already inside the team. A stranger with no account reading an invitation is a different reader with a different answer, and both views already had a branch for naming nobody |
| 2026-08-26 | **The invitation's reply is the inviter's membership address, and its subject goes through the header strip** | Issue [#12](https://github.com/imonroe/brawling-mahogany/issues/12), round 3. Two small holes in the same mailable. `people.email` is a login credential and `team_memberships.email` is what a team recorded for somebody (IA §11's *"Person, not User"*), and the envelope was pairing the first with a display name taken from the second — two halves of one header reading different tables, and publishing a sign-in address to a stranger who has not accepted anything yet. And CLAUDE.md's *"every tenant string reaching a header goes through `headerSafe()`"* was false for exactly one caller: the subject interpolated `$team->name` raw while the From name beside it was stripped. A rule true of every case but one is the shape people stop checking, so the note now says to re-check it rather than repeat it |
| 2026-08-26 | **An address is validated by the parser that will send it** | Issue [#12](https://github.com/imonroe/brawling-mahogany/issues/12). `emily(work)@bosart.test` passes Laravel's `email` *and* `email:rfc` — a parenthesised comment is legal RFC 5322 — and throws when Symfony builds an `Address` from it, in a worker, after `action_instances.message_key` has been claimed. A claimed key is never retried (a provider can accept a message and then time out), so one address typed into Settings → Team killed every automated message the team sent, permanently, and the queue blamed the mail transport for a value that came from a form. `App\Rules\SendableEmailAddress` runs the real parser at save time and `SendingIdentity` skips an unparseable stored candidate — and the first version of that skip constructed *Laravel's* `Address`, a DTO that validates nothing, which is the same mistake made inside its own fix |
