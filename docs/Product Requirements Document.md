...
| SPF, DKIM, DMARC | **DKIM: likely.** SES verifies a domain through Easy DKIM CNAMEs, so `d=monroedigitalconsulting.com` signing is probably real. **SPF alignment and DMARC: unconfirmed, and neither follows from the other.** SPF is evaluated against the envelope MAIL FROM, and without a custom MAIL FROM domain in SES the Return-Path is `*.amazonses.com` — so the domain's own SPF record is never consulted and SPF is unaligned for DMARC, leaving DMARC to ride on DKIM alignment alone. DMARC itself is a `_dmarc` TXT record, entirely outside SES setup. #12's evidence says a domain is approved for sending and nothing about any of the three; check them in DNS **now**, because the first real send has already happened. Mail is going out before either was checked — so if SPF is unaligned (which it will be unless a custom MAIL FROM domain was configured in SES) a DMARC failure is already happening silently, rather than waiting to be found |
