...
**Where this actually stands (2026-08-26, #12).** SES is configured and `monroedigitalconsulting.com` is verified; every message leaves as `goldieflow@monroedigitalconsulting.com` over SMTP. Three of the seven requirements above are therefore met or partly met, and the gaps are worth naming rather than leaving to be discovered:
...
| SPF, DKIM, DMARC | Verified on the domain as part of SES setup. DMARC policy strength not yet recorded here |
...
| Per-team sending identity | **Met, in the two slots that can carry it.** The address stays the one verified identity, because a `From` the domain is not authorised for fails SPF and DKIM; the team's own address rides in `Reply-To`; and the team's **name** goes in the inbox line as *"Bosart Group via Goldieflow"* — see `App\Support\Mail\SendingIdentity` |
