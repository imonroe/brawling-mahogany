...
| 2026-08-24 | **The person lifecycle describes a contact, not a colleague** (IA §8). A membership that carries team access and is not revoked is badged with its **roles**, and the lifecycle is neither shown nor editable for it | Slice 2, issue [#162](https://github.com/imonroe/brawling-mahogany/issues/162). `PersonLifecycleState`'s values are stages of a client relationship — Lead, Client, Past Client, Archived — and `AcceptInvitation` writes `active` onto a colleague's membership because the column has to hold something. `active`'s label is **Client**, so the directory told a team that their own assistant was a client of theirs. The same shape as the Vendor decision below: the answer is not a fifth lifecycle value but a different question, asked of what the membership *carries*. **Revocation ends it** — a former colleague is a person the team knows, and the lifecycle is exactly what describes them, so the rule asks `isColleague()` (access **and** not revoked) rather than `carriesAccess()` |
