...
| 2026-08-26 | **S91 sends one alert per team per hour, counts the whole backlog, and never alerts on a halt** | Slice 3, issue [#97](https://github.com/imonroe/brawling-mahogany/issues/97). A failed message is already a red row on S47 and an entry on the deal's timeline, and both are **pull** — true the moment somebody looks, and nobody looks at a queue that is usually empty. The failure this closes is a team believing a client was told, for a fortnight. One email per failure would send forty the first time a credential expires, so the first failure claims the hour and the alert counts everything currently failed. A **halt** is excluded deliberately: a halted message is still `pending`, nothing is lost, the sweep carries it when the condition clears, and both surfaces for it say so in the present tense. And it cannot throw — the commonest reason a message failed is the transport, which is the commonest reason the alert will fail, and a throw would fail the worker's job and retry it onto a row already marked failed |
